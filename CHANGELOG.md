...
### Added
- **Discussion Thread Creator Mentions & Welcome Message:** When an event discussion thread is automatically generated upon announcement posting, the bot now immediately posts an introductory welcome message inside the thread mentioning the event creator (`<@creatorId>`). This ensures event hosts are instantly notified that their event's discussion space is ready, and automatically adds them to the thread in Discord.

## [2.2.1] - 2026-08-23

### Fixed
- **Thread Inactivity BigInt Arithmetic Error:** Resolved a `TypeError: Cannot mix BigInt and other types` error during scheduled and manual thread pruning when calculating inactivity timestamps for active (unarchived) threads via `thread.lastMessageId` snowflakes.
...
### Added
- **Automatic Reminder Cleanup:** The bot now reliably sweeps and deletes old reminder messages from the public channel when the associated event concludes, significantly reducing channel clutter.
...
- **Legacy Recurring Event Rollover Announcements:** Fixed a bug where recurring events created before the bot was invited to a server would erroneously trigger a brand new "New Event" announcement precisely when they rolled over to their next occurrence.
