...
4. Go to "OAuth2" > "OAuth2 URL Generator". Check the `bot` and `applications.commands` scopes.
   - **Tip:** After checking the `bot` scope, a "Bot Permissions" grid will appear. It is highly recommended to select **Send Messages**, **Read Message History**, **View Channels**, **Embed Links**, and **Create Public Threads** (if using the auto-thread feature) so the bot's permissions are automatically configured.
